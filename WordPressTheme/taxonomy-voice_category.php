...
?>
    <div class="page-voice__tab common-tab">
      <ul class="common-tab__lists">
        <li class="common-tab__list <?php echo $cat_name === 'ALL' ? 'is-active' : ''; ?>"><a  href="<?php echo get_post_type_archive_link('voice') ?>">ALL</a></li>
        <?php $course_terms = get_terms('voice_category', array('hide_empty'=>false)); ?>
        <?php foreach($course_terms as $course_term ) : ?>
        <li class="common-tab__list <?php echo $cat_name === $course_term->name ? 'is-active' : ''; ?>"><a href="<?php echo get_term_link($course_term,'voice_category'); ?>"><?php echo $course_term->name; ?></a></li>
        <?php endforeach; ?>
      </ul>
    </div>
